...
public class RagService {
    ...
    private String buildMainSystemPrompt(String userId) {
        return String.format("""
                ...
                3. 승인/취소 등 도구(Tool) 호출 시 텍스트 형태의 기호나 특수 토큰을 만들어 기입하지 마세요.
                   - 승인 카드를 띄웠다고 안내할 때는, 사용자가 대상을 확인할 수 있도록 RAG 기반의 대상자 목록(이름, ID, 사유)을 반드시 텍스트로 함께 상세히 나열해 주세요.
                4. 여러 명에게 메일을 보낼 때는 반드시 sendBulkResultEmail 도구를 사용하세요.
                5. 텍스트로 특정 조건의 지원자 목록을 나열할 때는 **반드시 해당 지원자가 추출된 근거(요약)**를 그 옆에 한 줄로 작성해 주세요.
